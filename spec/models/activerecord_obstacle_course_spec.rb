...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '22. calculates the total sales for all but one user' do
    ...
    # ------------------ Using ActiveRecord ---------------------
    total_sales = User.where.not(id: 2).joins(:orders).sum(:amount)
    # -----------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:items).where("orders.user_id = ? AND order_items.item_id = ?", user_2.id, item_4.id)
    # -----------------------------------------------------------
